...
public class TransactionService(
    ...
    IValidator<CreateTransactionDto> validator) : ITransactionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await databaseTransaction.ExecuteAsync(async () =>
        {
            var transaction = new Transaction
            {
                ...
                Amount = dto.Amount,
                Type = dto.Type,
                Date = UtcDateHelper.ToUtcDate(dto.Date),
                Note = dto.Note,
                CreatedAt = DateTime.UtcNow
            };

            created = await transactionRepository.AddAsync(transaction, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        transaction.Date = UtcDateHelper.ToUtcDate(dto.Date);
    ...
    private static TransactionDto MapToDto(Transaction transaction) => new()
    {
        ...
        Tags = transaction.TransactionTags?
            ...
            .ToList() ?? [],
        TagIds = transaction.TransactionTags?
            .Select(tt => tt.TagId)
            .ToList() ?? []
    };
}
